...
import { deleteClassificationSurvey, getAllClassificationSurveys, getAllClients } from "@/api";
...
const Classifications = () => {
  ...
  const getAllClassification = async () => {
    try {
      ...
      const result = await getAllClassificationSurveys({ clientId: selectedShip.id, page: 1, limit: limit });
      ...
      renderCell: (params) => (
        <Stack direction="row" spacing={1}>
          <Tooltip title="Edit Classification">
            <IconButton color="primary" onClick={() => setEditRowId(params?.id)}>
              <EditIcon />
            </IconButton>
          </Tooltip>
    ...
    <Layout>
      ...
      <CommonCard>
        ...
        <Box sx={{ width: "100%", mt: 4 }}>
          {loading ? (
            <Box display="flex" justifyContent="center" alignItems="center" sx={{ height: 600, overflowY: "auto", mt: 2 }}>
              <CircularProgress />
            </Box>
      ...
      <Dialog open={openDialog} onClose={handleCancelDelete}>
        <DialogTitle>Are you sure you want to delete this Classification?</DialogTitle>
        <DialogActions>
