      ...
      {categoryFields?.map((field) => {
        ...
        return (
          <Grid2 size={{ xs: 12, sm: 6, md: 4 }} key={attr}>
            <TextField variant="outlined" fullWidth title={field.label} size="small" label={field.label} value={isDate ? formatDate(formData[attr]) : formData[attr] || ""} onChange={(e) => handleInputChange(attr, e.target.value)} placeholder={field.label} type={isDate ? "date" : "text"} InputLabelProps={isDate ? { shrink: true } : undefined} />
          </Grid2>
        );
      })}
    </Grid2>
  ...
  const renderCategoryAccordion = (title, key, icon, categoryFields) => {
    ...
    return (
      <Accordion expanded={expandedSection === key} onChange={() => setExpandedSection(expandedSection === key ? null : key)} sx={{ mb: 2 }}>
            ...
            <Typography component="span" variant="body2" color="primary" sx={{ ml: 1, fontWeight: "medium" }}>
              (
              {
                categoryFields?.filter((f) => {
                  const value = formData[f.attribute];
        ...
        <AccordionDetails>{renderFieldCategory(categoryFields)}</AccordionDetails>
      </Accordion>
    );
  };
