...
const InternationalTonnage = ({ open, onClose, onSubmit, fields, reportDetails }) => {
  ...
  const organizeFields = () => {
    ...
    fields?.forEach((field) => {
      ...
      if (attr?.includes("_gt_space_") || attr?.includes("_gt_loc_") || attr?.includes("_gt_length_") || attr?.includes("_nt_space_") || attr?.includes("_nt_loc_") || attr?.includes("_nt_length_")) {
        return;
      }
      ...
      } else if (attr?.includes("built") || attr?.includes("mast") || attr?.includes("deck") || attr?.includes("material") || attr?.includes("stem") || attr?.includes("stern")) {
      ...
      <Table size="small">
        ...
        <TableBody>
          {Array.from({ length: Math.max(gtSpaces.length, ntSpaces.length) }, (_, index) => (
            ...
            >
              <TableCell sx={{ borderRight: "1px solid #e0e0e0" }}>{gtSpaces[index] && <TextField fullWidth label={getLabelFromFields(gtSpaces[index].spaceAttr)} size="small" type={isDateField(gtSpaces[index].spaceAttr) ? "date" : "text"} value={isDateField(gtSpaces[index].spaceAttr) ? formatDate(formData[gtSpaces[index].spaceAttr]) : formData[gtSpaces[index].spaceAttr] || ""} onChange={(e) => handleInputChange(gtSpaces[index].spaceAttr, e.target.value)} />}</TableCell>
              <TableCell sx={{ borderRight: "1px solid #e0e0e0" }}>{gtSpaces[index] && <TextField fullWidth label={getLabelFromFields(gtSpaces[index].locAttr)} size="small" type={isDateField(gtSpaces[index].locAttr) ? "date" : "text"} value={isDateField(gtSpaces[index].locAttr) ? formatDate(formData[gtSpaces[index].locAttr]) : formData[gtSpaces[index].locAttr] || ""} onChange={(e) => handleInputChange(gtSpaces[index].locAttr, e.target.value)} />}</TableCell>
              <TableCell sx={{ borderRight: "2px solid #dee2e6" }}>{gtSpaces[index] && <TextField fullWidth label={getLabelFromFields(gtSpaces[index].lengthAttr)} size="small" value={isDateField(gtSpaces[index].lengthAttr) ? formatDate(formData[gtSpaces[index].lengthAttr]) : formData[gtSpaces[index].lengthAttr] || ""} onChange={(e) => handleInputChange(gtSpaces[index].lengthAttr, e.target.value)} placeholder="Length" />}</TableCell>

              {/* NT Fields */}
              <TableCell sx={{ borderRight: "1px solid #e0e0e0" }}>{ntSpaces[index] && <TextField fullWidth label={getLabelFromFields(ntSpaces[index].spaceAttr)} size="small" type={isDateField(ntSpaces[index].spaceAttr) ? "date" : "text"} value={isDateField(ntSpaces[index].spaceAttr) ? formatDate(formData[ntSpaces[index].spaceAttr]) : formData[ntSpaces[index].spaceAttr] || ""} onChange={(e) => handleInputChange(ntSpaces[index].spaceAttr, e.target.value)} placeholder="Space name" />}</TableCell>
              <TableCell sx={{ borderRight: "1px solid #e0e0e0" }}>{ntSpaces[index] && <TextField fullWidth label={getLabelFromFields(ntSpaces[index].locAttr)} size="small" type={isDateField(ntSpaces[index].locAttr) ? "date" : "text"} value={isDateField(ntSpaces[index].locAttr) ? formatDate(formData[ntSpaces[index].locAttr]) : formData[ntSpaces[index].locAttr] || ""} onChange={(e) => handleInputChange(ntSpaces[index].locAttr, e.target.value)} placeholder="Location" />}</TableCell>
              <TableCell>{ntSpaces[index] && <TextField fullWidth label={getLabelFromFields(ntSpaces[index].lengthAttr)} size="small" type={isDateField(ntSpaces[index].lengthAttr) ? "date" : "text"} value={isDateField(ntSpaces[index].lengthAttr) ? formatDate(formData[ntSpaces[index].lengthAttr]) : formData[ntSpaces[index].lengthAttr] || ""} onChange={(e) => handleInputChange(ntSpaces[index].lengthAttr, e.target.value)} placeholder="Length" />}</TableCell>
            </TableRow>
          ))}
        </TableBody>
      </Table>
        ...
        {categoryFields.map((field) => {
          ...
          return (
            <Grid2 item xs={12} sm={6} md={3} key={attr}>
              <TextField fullWidth type={isDateField(attr) ? "date" : "text"} label={formatLabel(field.label)} size="small" value={isDateField(attr) ? formatDate(value) : value || ""} onChange={(e) => handleInputChange(attr, e.target.value)} placeholder={`Enter ${formatLabel(field.label)?.toLowerCase()}`} InputLabelProps={isDateField(attr) ? { shrink: true } : undefined} />
            </Grid2>
          );
        })}
      </Grid2>
    ...
    >
      ...
      <DialogContent dividers sx={{ p: 3 }}>
        ...
        {fieldCategories.basic.length > 0 && (
          <Accordion expanded={expandedSection === "basic"} onChange={() => setExpandedSection(expandedSection === "basic" ? null : "basic")} sx={{ mb: 2 }}>
            <AccordionSummary expandIcon={<ExpandMoreIcon />}>
              <Typography variant="h6" sx={{ fontWeight: 500 }}>
                Basic Information
                <Typography component="span" variant="body2" color="primary" sx={{ ml: 1, fontWeight: "medium" }}>
                  ({fieldCategories.basic.filter((f) => formData[f.attribute]?.trim()).length}/{fieldCategories.basic.length})
                </Typography>
              </Typography>
            </AccordionSummary>
            <AccordionDetails>{renderFieldCategory(fieldCategories.basic)}</AccordionDetails>
          </Accordion>
        )}

        {/* Measurements Section */}
        {fieldCategories.measurements.length > 0 && (
          <Accordion expanded={expandedSection === "measurements"} onChange={() => setExpandedSection(expandedSection === "measurements" ? null : "measurements")} sx={{ mb: 2 }}>
            <AccordionSummary expandIcon={<ExpandMoreIcon />}>
              <Typography variant="h6" sx={{ fontWeight: 500 }}>
                Measurements & Dimensions
                <Typography component="span" variant="body2" color="primary" sx={{ ml: 1, fontWeight: "medium" }}>
                  ({fieldCategories.measurements.filter((f) => formData[f.attribute]?.trim()).length}/{fieldCategories.measurements.length})
                </Typography>
              </Typography>
            </AccordionSummary>
            <AccordionDetails>{renderFieldCategory(fieldCategories.measurements)}</AccordionDetails>
          </Accordion>
        )}

        {/* Spaces Information Section */}
        <Accordion expanded={expandedSection === "spaces"} onChange={() => setExpandedSection(expandedSection === "spaces" ? null : "spaces")} sx={{ mb: 2 }}>
          <AccordionSummary expandIcon={<ExpandMoreIcon />}>
            <Typography variant="h6" sx={{ fontWeight: 500 }}>
              Spaces Included in Tonnage
              <Typography component="span" variant="body2" color="primary" sx={{ ml: 1, fontWeight: "medium" }}>
                ({[...gtSpaces, ...ntSpaces].filter((space) => formData[space.spaceAttr]?.trim() || formData[space.locAttr]?.trim() || formData[space.lengthAttr]?.trim()).length}/{gtSpaces.length + ntSpaces.length})
              </Typography>
            </Typography>
          </AccordionSummary>
          <AccordionDetails sx={{ p: 0 }}>{renderSpacesTable()}</AccordionDetails>
        </Accordion>

        {/* Passenger Information Section */}
        {fieldCategories.passengers.length > 0 && (
          <Accordion expanded={expandedSection === "passengers"} onChange={() => setExpandedSection(expandedSection === "passengers" ? null : "passengers")} sx={{ mb: 2 }}>
            <AccordionSummary expandIcon={<ExpandMoreIcon />}>
              <Typography variant="h6" sx={{ fontWeight: 500 }}>
                Passenger Information
                <Typography component="span" variant="body2" color="primary" sx={{ ml: 1, fontWeight: "medium" }}>
                  ({fieldCategories.passengers.filter((f) => formData[f.attribute]?.trim()).length}/{fieldCategories.passengers.length})
                </Typography>
              </Typography>
            </AccordionSummary>
            <AccordionDetails>{renderFieldCategory(fieldCategories.passengers)}</AccordionDetails>
          </Accordion>
        )}

        {/* Construction Details Section */}
        {fieldCategories.construction.length > 0 && (
          <Accordion expanded={expandedSection === "construction"} onChange={() => setExpandedSection(expandedSection === "construction" ? null : "construction")} sx={{ mb: 2 }}>
            <AccordionSummary expandIcon={<ExpandMoreIcon />}>
              <Typography variant="h6" sx={{ fontWeight: 500 }}>
                Construction Details
                <Typography component="span" variant="body2" color="primary" sx={{ ml: 1, fontWeight: "medium" }}>
                  ({fieldCategories.construction.filter((f) => formData[f.attribute]?.trim()).length}/{fieldCategories.construction.length})
                </Typography>
              </Typography>
            </AccordionSummary>
            <AccordionDetails>{renderFieldCategory(fieldCategories.construction)}</AccordionDetails>
          </Accordion>
        )}

        {/* Dates & Measurements Section */}
        {fieldCategories.dates.length > 0 && (
          <Accordion expanded={expandedSection === "dates"} onChange={() => setExpandedSection(expandedSection === "dates" ? null : "dates")} sx={{ mb: 2 }}>
            <AccordionSummary expandIcon={<ExpandMoreIcon />}>
              <Typography variant="h6" sx={{ fontWeight: 500 }}>
                Dates & Historical Data
                <Typography component="span" variant="body2" color="primary" sx={{ ml: 1, fontWeight: "medium" }}>
                  ({fieldCategories.dates.filter((f) => formData[f.attribute]?.trim()).length}/{fieldCategories.dates.length})
                </Typography>
              </Typography>
            </AccordionSummary>
            <AccordionDetails>{renderFieldCategory(fieldCategories.dates)}</AccordionDetails>
          </Accordion>
        )}

        {/* Other Information Section */}
        {fieldCategories.other.length > 0 && (
          <Accordion expanded={expandedSection === "other"} onChange={() => setExpandedSection(expandedSection === "other" ? null : "other")} sx={{ mb: 2 }}>
            <AccordionSummary expandIcon={<ExpandMoreIcon />}>
              <Typography variant="h6" sx={{ fontWeight: 500 }}>
                Other Information
                <Typography component="span" variant="body2" color="primary" sx={{ ml: 1, fontWeight: "medium" }}>
                  ({fieldCategories.other.filter((f) => formData[f.attribute]?.trim()).length}/{fieldCategories.other.length})
                </Typography>
              </Typography>
            </AccordionSummary>
            <AccordionDetails>{renderFieldCategory(fieldCategories.other)}</AccordionDetails>
          </Accordion>
        )}
      </DialogContent>
