...
export const generateChecklistHTML = (item, companyName, companyLogo) => {
    ...
    const cleanedHTML = cleanHTMLContent(originalHTML);

    const htmlDetails = extractShipDetailsFromHTML(cleanedHTML);

    const shipName = htmlDetails.shipName || item?.client?.shipName || 'N/A';
    const reportNo = item?.journal?.journalTypeId || 'N/A';
    const surveyType = item?.surveyType?.name || 'Survey Report';
    const imoNo = htmlDetails.imoNo || item?.reportNo || 'N/A';
    const irNo = htmlDetails.irNo || item?.reportNo || 'N/A';
    const portOfSurvey = htmlDetails.portOfSurvey || 'N/A';
...
<head>
  <meta charset="UTF-8">
  <title>Survey Checklist Report - ${shipName}</title>
  <style>
    @page {
      margin: 20mm;
      size: A4;
...
max-width:140px;
    ...
    /* Hide the original ship details table in content */
...
<body>
  ...
  <div class="document-header">
    <div class="header-top">
      ${companyLogo ? `<img src="${companyLogo}" alt="Company Logo" class="company-logo" />` : '<div style="width: 150px;"></div>'}
      
      <div class="header-center">
        <h2 class="document-title">Checklist Report</h2>
        <div class="document-subtitle">${surveyType}</div>
      </div>
